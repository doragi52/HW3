...
static int tempseed;
long random(int max) //random lottery select
{
        srand(tempseed);
        tempseed++;
        int winner = rand() % max;
...
scheduler(void)
{
  ...
  for(;;){
    ...
    sti();
    int sumticket = 0; // total ticket's sum
    int count = 0; //where ticket
    // init sum & count  
